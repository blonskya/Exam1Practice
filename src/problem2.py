...
def problem2a(circle, rectangle, window):
    ...
    window.render(0.05)
    window.continue_on_mouse_click()
    nwlne=rg.Line(rectangle.get_upper_right_corner(),rectangle.get_lower_left_corner())
    nwlne.arrow='last'
    nwlne.attach_to(window)
    window.render(0.05)
    window.continue_on_mouse_click()
...
def problem2b(rect, n, delta, win):
    ...
    for k in range (n):
        rangle2=rangle
        rangle2.attach_to(win)
        win.render(0.05)
        if rangle.corner_1.x<rangle.corner_2.x:
            rangle.corner_1.x -= delta
            rangle.corner_2.x += delta
        else:
            rangle.corner_1.x += delta
            rangle.corner_2.x -= delta
        if rangle.corner_1.y < rangle.corner_2.y:
            rangle.corner_1.y -= delta
            rangle.corner_2.y += delta
        else:
            rangle.corner_1.y += delta
            rangle.corner_2.y -= delta
    rangle2 = rangle
